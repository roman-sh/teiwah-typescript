...
import { TeiwahDefaultError } from "../models/errors/teiwah-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | TeiwahDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | TeiwahDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new TeiwahDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new TeiwahDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
